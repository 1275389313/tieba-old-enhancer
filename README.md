...
| 脚本 | 版本 | 说明 |
|------|------|------|
| [tieba-long-image-auto-expand.user.js](tieba-long-image-auto-expand.user.js) | 0.3.2 | ① 自动展开长图为完整图片；② 自动展开全部楼中楼回复（免点"查看更多回复"）；③ 隐藏页面右侧推广/辅助按钮（辅助模式、下载APP、魔法道具、神来一句、分享、我要反馈）及"爱逛的吧"模块，并监听 AJAX 动态加载的楼层 |
...
脚本已声明 `@updateURL`（指向 GitHub 原始文件，实时返回最新版本号）和 `@downloadURL`（指向 jsDelivr CDN，国内可直接下载）。油猴按设置的更新周期自动检查版本号（`@version`）变化，发现新版本后提示更新；也可以随时在油猴菜单栏点击脚本检查更新。发布新版本时只需修改 `@version` 号并推送仓库即可。
